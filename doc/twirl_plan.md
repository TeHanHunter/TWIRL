...
1. Run BLS/heuristic/LEO checks on `/pdo/users/tehan/tglc-gpu-production/hlsp_s0056_twirl_fs_v1` and compare against the prior QLP/TWIRL S56 products.
2. Complete product QA for TWIRL-FS v1 with full-sample cadence retention, RMS/MAD, quality flags, WD 1856 timing, and injected short-event preservation.
3. Decide whether `twirl-fs-v1` replaces the normal QLP detrending path for production sectors, and if yes, refresh/relabel downstream FITS trees consistently.
4. Build the v2/v3/TWIRL-FS comparison table only after the light-curve product is clearly validated.
...
8. Lock down follow-up readiness and coordination only after the candidate-validation criteria are stable.
