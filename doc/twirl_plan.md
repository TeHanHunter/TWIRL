...
- `2026-07-10`: **S56 A2v1 reproduction and product validation are complete.** The clean A2v1 root at `/pdo/users/tehan/tglc-gpu-production-A2v1/` has `3,136` ePSFs per orbit, `62,935` HDF5 light curves, and `31,450` ADP/ADP015-only FITS products. All `62,512` non-edge observation rows emitted HDF5 light curves; the `303` remaining raw-table gaps and `140` absent FITS TICs are all documented `edge_warn` aperture exclusions. A bounded-parallel full-schema audit opened every FITS and found `0` bad schemas. The reusable [launcher](../scripts/stage1_lightcurves/run_a2v1_reproduction_pdo.sh), [validator](../scripts/stage1_lightcurves/validate_a2v1_product.py), and [production protocol](a2v1_production_protocol.md) are ready for S94+ default-source reuse; photometric QA remains the gate before wider survey finalization.
- `2026-07-10`: **S57 is now exercising the A2v1 protocol across all CCDs.** Full orbit `121` + `122` reproduction launched on `pdogpu6` in tmux `twirl-s57-a2v1-r1`, reusing prepared sources and empty-mask ePSFs while refitting masked cutouts with `epsfs-nprocs=2`. The S56 contract and [A2v1 production protocol](a2v1_production_protocol.md) are the acceptance criteria; this is production validation, not yet a photometric-QA sign-off.
...
- `2026-07-10`: **The resolved post-adjudication teacher contract is `s56_harmonic_cnn_v1`.** The four morphology outputs are Planet-like, Eclipse/contact, Smooth variable, and Other; the `14` Broad isolated dips remain preserve-positive/harmonic-audit rows until that class reaches `50`. Training uses all `307` human-visible injected planets plus `12` real planets, with equal aggregate real/injected Planet loss weight, while injection truth never overwrites morphology. Native raw/error and ADP-small/ADP inputs, seven BLS-harmonic folds, local primary/secondary windows, two-aperture BLS periodograms, and scalar BLS metadata are separate branches; human-corrected factors supervise the auxiliary harmonic head for real rows, while the known truth-to-BLS period ratio provides that auxiliary target for injected rows only. ORCD Torch validation and a one-H200 end-to-end lifecycle smoke pass; production training waits only for the compact raw/error host export from the matching PDO TGLC tree. Student pseudo-labeling remains blocked until at least `50` unique real Planet-like examples exist.
