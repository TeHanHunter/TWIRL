...
- `2026-07-11`: **The post-adjudication `s56_harmonic_cnn_v1` teacher is trained and audited.** The exact native input product contains `1,696` real targets plus `456` active injections with raw/error, two ADP apertures, and two-aperture periodograms. A one-H200, six-profile, five-fold run selected Shape + periodogram/BLS (development macro F1 `0.807`) over full combined (`0.795`) and metadata-only (`0.724`). On the once-opened locked morphology test it reaches balanced accuracy `0.750`, macro F1 `0.757`, and ECE `0.048`; real-only balanced accuracy is `0.838`, but only `2` real Planet-like examples are in the test set, so this is an active-learning teacher rather than a final student-label generator. See the [model summary](../reports/stage5_validation/s56_label_adjudication_real343/retrained/s56_harmonic_cnn_v1/model_summary.md) and [performance figure](../reports/stage5_validation/s56_label_adjudication_real343/retrained/s56_harmonic_cnn_v1/performance_figure/s56_harmonic_cnn_v1_performance.png). Student pseudo-labeling remains blocked until at least `50` unique real Planet-like examples exist.
...
Status (`2026-07-11`, production harmonic teacher): the strict adjudication
and native-input gates are complete, and ORCD job `17655871` trained all six
profiles across five TIC-grouped development folds on one H200. The selector
chose Shape + periodogram/BLS; adding raw chronological branches was tested but
ranked second in the full-combined profile. The locked test has `428` active
morphology rows (`337` real and `91` injected), with balanced accuracy `0.750`,
macro F1 `0.757`, and ECE `0.048`. The model passes its configured smoke gates,
