# Archived TWIRL ideas through 2026-07-13

This frozen snapshot preserves brainstorming through `2026-07-13`. The live
unresolved-question list is [`doc/ideas.md`](../ideas.md).
...
### Teacher-v2 harmonic ambiguity

The V1 harmonic CNN saw `P/4`, `P/3`, `P/2`, `P`, `2P`, `3P`, and `4P`, but
rare-factor competence remains unresolved because the harmonic targets were
heavily concentrated at `P`; `P/3` had no supervised example and `3P` had only
three. Supplying a view is not equivalent to teaching the model when to use it.
Teacher v2 should test explicit per-factor morphology predictions combined by
the harmonic head against the V1 attention-pooling design.

Open diagnostics before promoting V2:

- Measure how often real A2v1 candidates require `P/3` or `3P`, separately for
  Planet-like and eclipse/contact labels, rather than inferring prevalence from
  injections alone.
- Compare predicted-factor morphology with reviewed/truth-oracle-factor
  morphology. A large gap means period selection is failing; a small gap means
  the morphology encoder or label support is the limitation.
- Verify that training-only reference-anchor augmentation improves rare-factor
  recovery without creating injection/provenance shortcuts. Keep the locked
  test anchored to actual ADP-small BLS outputs.
- Keep `morphology_fold_factor` separate from physical orbital-period inference,
  especially for binaries where the best diagnostic primary-plus-secondary fold
  and the eclipse-recurrence period need not express the same scientific claim.
