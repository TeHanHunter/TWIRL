...
def label_peak_against_injection(
    *,
    ...
    harmonic_t0_err = _phase_error_min(t0, truth_t0, min(period, truth_period))
...
def peak_rows_for_injection(
    payload: tuple[str, str, tuple[str, ...], dict[str, Any], float, tuple[float, ...]]
) -> list[dict[str, Any]]:
    h5_path_s, group_path, requested_apertures, cfg_kwargs, period_tol, harmonic_factors = payload
    ...
    for aperture in apertures:
        try:
            ...
            peak_rows = [
                row
                for row in result_to_rows(result, run_id="s56-injection-peak-training")
...
def build_peak_training_table(
    *,
    ...
    payloads = [
        (str(injection_h5), f"/injections/{key}", apertures, cfg_kwargs, float(period_tol), harmonic_factors)
        for key in keys
    ]
    print(
        f"[peak-training] running BLS for {len(payloads):,} injections, "
        f"{len(apertures)} requested apertures, top {n_peaks} peaks",
        flush=True,
    )
    rows: list[dict[str, Any]] = []
