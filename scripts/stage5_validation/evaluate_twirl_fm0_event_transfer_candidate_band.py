#!/usr/bin/env python3
"""Run the fixed-band cadence-preserving TWIRL-FM transfer screen."""
...
from twirl.models.fm0.event_transfer_candidate_band import (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mechanics-result", type=Path, required=True)
...
def main() -> int:
    ...
    if args.output.exists() or sidecar.exists():
        raise FileExistsError("refusing to overwrite candidate-band output")
    require_clean_git_revision(ROOT, args.expected_git_sha)
    payload = evaluate_event_transfer_candidate_band(
        config_path=args.config,
        mechanics_result_path=args.mechanics_result,
