#!/usr/bin/env python3
"""Run the CPU-only fixed-exposure TWIRL-FM context diagnostic."""
...
from twirl.models.fm0.context_window_diagnostic import (
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--max-repeated-components", type=int, default=64)
    parser.add_argument("--max-new-components", type=int, default=64)
...
def main() -> int:
    ...
    if args.output.exists() or sidecar.exists():
        raise FileExistsError("refusing to overwrite context diagnostic output")
    require_clean_git_revision(ROOT, args.expected_git_sha)
    payload = evaluate_context_window_diagnostic(
