...
INJECTION_DIR="${INJECTION_DIR:-data_local/stage3_injections/s56_twirlfs_v2_injection_training/pdo_1k_batman_depthgrid_r1p5j_1000}"
REVIEW_DIR="${REVIEW_DIR:-reports/stage5_validation/s56_1k_batman_depthgrid_r1p5j_review_queue_pdo}"
...
RANDOM_STATE="${RANDOM_STATE:-5601}"
REBUILD_EXPORT="${REBUILD_EXPORT:-0}"
REBUILD_INJECTIONS="${REBUILD_INJECTIONS:-1}"
REUSE_INJECTION_BLS="${REUSE_INJECTION_BLS:-0}"
SKIP_LEO="${SKIP_LEO:-0}"

echo "[pdo-1k-batman-review] start $(date -Is)"
echo "[pdo-1k-batman-review] python=${PYTHON_BIN}"
echo "[pdo-1k-batman-review] review_dir=${REVIEW_DIR}"
...
if [[ "${REBUILD_EXPORT}" == "1" || ! -s "${EXPORT_H5}" ]]; then
  ...
  echo "[pdo-1k-batman-review] reusing compact export: ${EXPORT_H5}"
fi
...
if [[ "${REBUILD_INJECTIONS}" == "1" || ! -s "${INJECTION_DIR}/injected_lightcurves.h5" ]]; then
  ...
  echo "[pdo-1k-batman-review] reusing injections: ${INJECTION_DIR}/injected_lightcurves.h5"
fi
...
echo "[pdo-1k-batman-review] complete $(date -Is)"
echo "[pdo-1k-batman-review] review queue: ${REVIEW_DIR}/review_queue.csv"
