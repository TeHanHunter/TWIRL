...
INJECTION_DIR="$(dirname "${INJECTION_H5}")"
INJECTION_MANIFEST="${INJECTION_MANIFEST:-${INJECTION_DIR}/injection_manifest.csv}"
...
log "injection_manifest=${INJECTION_MANIFEST}"
...
"${PYTHON}" - "${INJECTION_H5}" "${INJECTION_MANIFEST}" "${CHUNK_DIR}" "${CHUNK_SIZE}" "${N_INJECTIONS}" <<'PY'
...
injection_manifest = Path(sys.argv[2])
chunk_dir = Path(sys.argv[3])
chunk_size = int(sys.argv[4])
n_injections = int(sys.argv[5])

keys = []
key_source = "hdf5"
if injection_manifest.exists():
    with injection_manifest.open(newline="", encoding="utf-8") as fh:
        reader = csv.DictReader(fh)
        for row in reader:
            group = row.get("h5_group") or row.get("injection_id") or ""
            group = group.strip()
            if group:
                keys.append(PurePosixPath(group).name)
    key_source = "manifest"
