...
def _export_group_key(tic: int) -> str:
    return f"targets/{int(tic):016d}"


def _fallback_export_flux(export_h5: Path | None, *, tic: int, aperture: str, target_time: np.ndarray) -> np.ndarray | None:
    if export_h5 is None or not export_h5.exists():
        return None
    with h5py.File(export_h5, "r") as h5:
        key = _export_group_key(tic)
        if key not in h5:
            return None
        group = h5[key]
        if aperture not in group:
            return None
        return _align_flux_to_time(
            np.asarray(group["time"], dtype=np.float64),
            np.asarray(group[aperture], dtype=np.float64),
            target_time,
        )


def _export_lc(export_h5: Path | None, tic: int, apertures: tuple[str, ...]) -> HLSPLightCurve | None:
    if export_h5 is None or not export_h5.exists():
        return None
    with h5py.File(export_h5, "r") as h5:
        key = _export_group_key(tic)
        ...
        group = h5[key]
        flux = {
            aperture: np.asarray(group[aperture], dtype=np.float64)
            for aperture in apertures
            if aperture in group
        }
        if not flux:
            return None
        payload = {
            ...
            "cadenceno": np.asarray(group["cadenceno"], dtype=np.int32) if "cadenceno" in group else np.arange(len(group["time"]), dtype=np.int32),
            "orbitid": np.asarray(group["orbitid"], dtype=np.int16),
            "quality": np.asarray(group["quality"], dtype=np.int32),
            "flux": flux,
            "path": Path(f"{export_h5}:{key}"),
        }
...
def _h5_lc(
    ...
    *,
    injected: bool,
    hlsp_root: Path | None = None,
    export_h5: Path | None = None,
) -> HLSPLightCurve:
    with h5py.File(path, "r") as h5:
        ...
        if dataset in group:
            ...
            if original_name in group:
                ...
                base_flux = _fallback_export_flux(export_h5, tic=tic, aperture=aperture, target_time=time)
...
def _run_branch(
    ...
    *,
    ...
    iterative_anchor: tuple[float, float, float] | None = None,
    ...
    elif mask_mode == "iterative" and iterative_anchor is not None:
        period, t0, duration = iterative_anchor
...
def _top_peak_anchor(res: Any) -> tuple[float, float, float]:
    if getattr(res, "peaks", None):
        peak = res.peaks[0]
        return float(peak.period_d), float(peak.t0_bjd), float(peak.duration_min)
...
def _process_injection(payload: tuple[dict[str, Any], str, str, str, list[str], list[str], int, int]) -> list[dict[str, Any]]:
    row, h5_path_s, hlsp_root_s, export_h5_s, branch_names, mask_modes, n_periods, n_peaks = payload
    h5_path = Path(h5_path_s)
    hlsp_root = Path(hlsp_root_s) if hlsp_root_s else None
    export_h5 = Path(export_h5_s) if export_h5_s else None
    ...
    for aperture in ADP_VET_APERTURES:
        try:
            inj_lc = _h5_lc(h5_path, str(row["h5_group"]), aperture, injected=True, hlsp_root=hlsp_root, export_h5=export_h5)
            try:
                orig_base = _h5_lc(h5_path, str(row["h5_group"]), aperture, injected=False, hlsp_root=hlsp_root, export_h5=export_h5)
            except Exception:
                orig_base = None
            ...
            continue
        current_cache = None
        current_anchor: tuple[float, float, float] | None = None
        try:
            current_cache = _run_branch(
                inj_lc,
                aperture=aperture,
                branch=branch_by_name("current_adp"),
                mask_mode="current",
                cfg=cfg,
                truth=truth,
            )
            current_anchor = _top_peak_anchor(current_cache[0])
        except Exception:
            current_cache = None
            current_anchor = None
        for branch_name in branch_names:
            ...
            for mode in modes:
                try:
                    if branch.is_identity and mode == "current" and current_cache is not None:
                        res, meta, branch_lc = current_cache
                    else:
                        res, meta, branch_lc = _run_branch(
                            inj_lc,
                            aperture=aperture,
                            branch=branch,
                            mask_mode=mode,
                            cfg=cfg,
                            truth=truth,
                            iterative_anchor=current_anchor,
                        )
                    orig_branch_lc = None
                    if orig_base is not None:
                        try:
                            ...
                            elif mode == "iterative" and current_anchor is not None:
                                orig_branch_lc, _ = branched_light_curve(
                                    orig_base,
                                    aperture,
                                    branch,
                                    period_d=current_anchor[0],
                                    t0_bjd=current_anchor[1],
                                    duration_min=current_anchor[2],
                                )
                        except Exception:
                            orig_branch_lc = None
...
def _process_real(payload: tuple[dict[str, Any], str, str, list[str], int, int]) -> list[dict[str, Any]]:
    row, hlsp_root_s, export_h5_s, branch_names, n_periods, n_peaks = payload
    ...
    path = find_hlsp_path(hlsp_root, tic, sector)
    lc = read_hlsp(path, columns=ADP_VET_APERTURES) if path is not None else None
    if lc is None:
        lc = _export_lc(export_h5, tic, ADP_VET_APERTURES)
    if lc is None:
        return [{"source_kind": "real_candidate", "tic": tic, "status": "missing_lightcurve"}]
    cfg = BLSConfig(apertures=ADP_VET_APERTURES, n_periods=n_periods, n_peaks=n_peaks)
    records: list[dict[str, Any]] = []
    for aperture in ADP_VET_APERTURES:
...
def run_audit(args: argparse.Namespace) -> dict[str, Any]:
    ...
    inj_payloads = [
        (
            ...
            str(args.hlsp_root),
            str(args.export_h5) if args.export_h5 else "",
            branches,
            mask_modes,
            int(args.n_periods),
            int(args.n_peaks),
    ...
    real_payloads = [
        (row, str(args.hlsp_root), str(args.export_h5) if args.export_h5 else "", branches, int(args.n_periods), int(args.n_peaks))
    ...
    summary = {
        ...
        "export_h5": str(args.export_h5) if args.export_h5 else "",
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--export-h5",
        type=Path,
        default=DEFAULT_EXPORT_H5,
        help="Optional compact LC export used before HLSP FITS fallback.",
    )
