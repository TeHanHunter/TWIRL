...
DEFAULT_EXPORT_H5 = Path("data_local/stage3_injections/s56_twirlfs_v2_lc_export/s56_twirlfs_v2_adp_lc_export_pdo.h5")
...
def _read_export_lc(export_h5: Path | None, *, tic: int, columns: tuple[str, ...]) -> HLSPLightCurve | None:
    ...
    with h5py.File(export_h5, "r") as h5:
        ...
        orbitid = np.asarray(group["orbitid"], dtype=np.int16) if "orbitid" in group else np.zeros(len(time), dtype=np.int16)
        cadenceno = np.asarray(group["cadenceno"], dtype=np.int32) if "cadenceno" in group else np.arange(len(time), dtype=np.int32)
        flux = {col: np.asarray(group[col], dtype=np.float64) for col in columns if col in group}
        if not flux:
            return None
        payload = {
            ...
            "quality": quality,
            "flux": flux,
            "path": Path(f"{export_h5}:targets/{key}"),
        }
...
def run_check(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if lc is None:
        lc = _read_export_lc(args.export_h5, tic=args.tic, columns=ADP_VET_APERTURES)
    if lc is None:
        raise FileNotFoundError(f"no HLSP/export light curve found for TIC {args.tic}")
        ...
        for branch_name in branches:
            ...
            for mode in modes:
                ...
                context = {
                    ...
                    "hlsp_path": str(path) if path is not None else "",
                    "lc_path": str(lc.path),
                }
                rows.extend(
                    _peak_rows(
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--export-h5", type=Path, default=DEFAULT_EXPORT_H5)
