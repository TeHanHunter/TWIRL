#!/usr/bin/env bash
# Rebuild human-label audit products for the S56 mixed-teacher first-pass queue.
#
# Run this after labeling rows in run_s56_mixed_teacher_vetting_app_pdo.sh or
# after syncing local labels back to PDO.
set -euo pipefail
...
NEXT_N_ROWS="${NEXT_N_ROWS:-1000}"
TARGET_PER_CELL="${TARGET_PER_CELL:-25}"

if [[ ! -s "${VERIFY_JSON}" ]]; then
  echo "[mixed-teacher-label-audit] missing verifier: ${VERIFY_JSON}" >&2
  exit 2
fi
...
PY
then
  echo "[mixed-teacher-label-audit] verifier did not pass: ${VERIFY_JSON}" >&2
  exit 2
fi
if [[ ! -s "${LABELS_CSV}" ]]; then
  echo "[mixed-teacher-label-audit] missing labels: ${LABELS_CSV}" >&2
  echo "[mixed-teacher-label-audit] label rows in the vetting app before running this audit" >&2
  exit 2
fi

echo "[mixed-teacher-label-audit] queue=${QUEUE_CSV}"
echo "[mixed-teacher-label-audit] labels=${LABELS_CSV}"
echo "[mixed-teacher-label-audit] label_summary=${LABEL_SUMMARY_DIR}"
echo "[mixed-teacher-label-audit] training_table=${TRAINING_DIR}"
echo "[mixed-teacher-label-audit] priority=${PRIORITY_DIR}"
echo "[mixed-teacher-label-audit] readiness=${READINESS_DIR}"
...
echo "[mixed-teacher-label-audit] complete"
echo "[mixed-teacher-label-audit] readiness summary: ${READINESS_DIR}/summary.md"
