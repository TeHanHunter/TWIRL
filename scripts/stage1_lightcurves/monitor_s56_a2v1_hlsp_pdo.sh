...
REPRO_START_BYTES=0
if [ -e "$REPRO_LOG" ]; then
  REPRO_START_BYTES=$(wc -c < "$REPRO_LOG" | tr -d " ")
fi
log "monitor will ignore reproduction log bytes <= $REPRO_START_BYTES"

repro_since_monitor_start() {
  if [ ! -e "$REPRO_LOG" ]; then
    return 0
  fi
  tail -c +"$((REPRO_START_BYTES + 1))" "$REPRO_LOG" 2>/dev/null || true
}

while true; do
  recent_repro=$(repro_since_monitor_start)

  if printf "%s" "$recent_repro" | grep -q "s56 A2v1 HDF5 reproduction complete"; then
  ...
  if printf "%s" "$recent_repro" | grep -q "ABORT:"; then
