...
case "${ACTION}" in
  ...
  prepare)
    probe >/dev/null
    "${SSH[@]}" "cd '${REPO}' && sbatch --parsable scripts/orcd/slurm_s56_a2v1_recovery_prepare_cpu.sbatch"
    ;;
  smoke)
    probe >/dev/null
    ...
    "${SSH[@]}" "cd '${REPO}' && TWIRL_RECOVERY_MODE=smoke sbatch --parsable --array=0 scripts/orcd/slurm_s56_a2v1_recovery_injection_bls_cpu.sbatch"
    ;;
  smoke-gpu)
    probe >/dev/null
    ...
    "${SSH[@]}" "cd '${REPO}' && TWIRL_RECOVERY_MODE=smoke sbatch --parsable scripts/orcd/slurm_s56_a2v1_recovery_teacher_h200.sbatch"
    ;;
  full)
    probe >/dev/null
    ...
    INJECT_JOB="$("${SSH[@]}" "cd '${REPO}' && TWIRL_RECOVERY_MODE=full sbatch --parsable scripts/orcd/slurm_s56_a2v1_recovery_injection_bls_cpu.sbatch")"
    NATIVE_JOB="$("${SSH[@]}" "cd '${REPO}' && sbatch --parsable --dependency=afterok:${INJECT_JOB} scripts/orcd/slurm_s56_a2v1_recovery_teacher_prepare_cpu.sbatch")"
    MERGE_JOB="$("${SSH[@]}" "cd '${REPO}' && sbatch --parsable --dependency=afterok:${NATIVE_JOB} scripts/orcd/slurm_s56_a2v1_recovery_merge_cpu.sbatch")"
    TEACHER_JOB="$("${SSH[@]}" "cd '${REPO}' && TWIRL_RECOVERY_MODE=full sbatch --parsable --dependency=afterok:${MERGE_JOB} scripts/orcd/slurm_s56_a2v1_recovery_teacher_h200.sbatch")"
    FINAL_JOB="$("${SSH[@]}" "cd '${REPO}' && sbatch --parsable --dependency=afterok:${TEACHER_JOB} scripts/orcd/slurm_s56_a2v1_recovery_finalize_cpu.sbatch")"
