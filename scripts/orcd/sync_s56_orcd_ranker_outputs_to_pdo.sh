#!/usr/bin/env bash
# Sync small ORCD ranker-selected real-candidate outputs back to PDO/local.
#
...
LOCAL_DEST_DIR="${TWIRL_LOCAL_SELECTED_DIR:-${SOURCE_DIR}}"
DRY_RUN=1
...
Copies ORCD ranker-selected real-candidate outputs to both the local checkout
and the PDO checkout. The PDO copy lets
scripts/stage5_validation/run_s56_ranker_selected_real_leo_pdo.sh render
...
echo "[sync-orcd-ranker] local=${LOCAL_REPO}/${LOCAL_DEST_DIR}"
if [[ "${DEST_DIR}" != "${SOURCE_DIR}" || "${LOCAL_DEST_DIR}" != "${SOURCE_DIR}" ]]; then
  echo "[sync-orcd-ranker] destination renaming is not supported for this guarded sync." >&2
  echo "[sync-orcd-ranker] set TWIRL_*_SELECTED_DIR values equal to ${SOURCE_DIR}." >&2
  exit 2
fi
if [[ "${DRY_RUN}" == "1" ]]; then
