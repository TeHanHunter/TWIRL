...
LEO_N_REAL="${TWIRL_PDO_LEO_N_REAL:-${TWIRL_REVIEW_N_REAL:-1000}}"
LEO_MAX_REPORTS="${TWIRL_PDO_LEO_MAX_REPORTS:-${TWIRL_MAX_LEO_REPORTS:-0}}"
LEO_WORKERS="${TWIRL_PDO_LEO_WORKERS:-${TWIRL_LEO_WORKERS:-8}}"
LEO_TIMEOUT_S="${TWIRL_PDO_LEO_TIMEOUT_S:-${TWIRL_LEO_TIMEOUT_S:-300}}"
...
if [[ "${RUN_PDO_LEO}" != "1" ]]; then
  ...
  log "next command:"
  cat >&2 <<EOF
ssh ${PDO_HOST} 'cd ${PDO_REPO} && TWIRL_SELECTED_EPHEMERIDES=${DEST_DIR}/selected_ephemerides.csv TWIRL_RANKER_LEO_OUT_DIR=${LEO_OUT_DIR} TWIRL_REVIEW_N_REAL=${LEO_N_REAL} TWIRL_MAX_LEO_REPORTS=${LEO_MAX_REPORTS} TWIRL_LEO_WORKERS=${LEO_WORKERS} TWIRL_LEO_TIMEOUT_S=${LEO_TIMEOUT_S} bash scripts/stage5_validation/run_s56_ranker_selected_real_leo_pdo.sh'
EOF
  exit 0
fi

log "launching PDO LEO rendering"
log "leo_out=${LEO_OUT_DIR}; n_real=${LEO_N_REAL}; max_leo_reports=${LEO_MAX_REPORTS}; workers=${LEO_WORKERS}"
ssh -o BatchMode=yes -o ConnectTimeout=20 "${PDO_HOST}" \
  "cd '${PDO_REPO}' && TWIRL_SELECTED_EPHEMERIDES='${DEST_DIR}/selected_ephemerides.csv' TWIRL_RANKER_LEO_OUT_DIR='${LEO_OUT_DIR}' TWIRL_REVIEW_N_REAL='${LEO_N_REAL}' TWIRL_MAX_LEO_REPORTS='${LEO_MAX_REPORTS}' TWIRL_LEO_WORKERS='${LEO_WORKERS}' TWIRL_LEO_TIMEOUT_S='${LEO_TIMEOUT_S}' bash scripts/stage5_validation/run_s56_ranker_selected_real_leo_pdo.sh"
log "complete"
