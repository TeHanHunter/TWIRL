...
# (cadence-fix + TWIRL-FS HLSP + audit + QC). Designed to run on pdogpu6.
...
HLSP_ROOT=$ROOT/hlsp_s00${SECTOR_TAG}_twirl_fs_v2
...
if [ "${TWIRL_SKIP_STEP_A:-0}" = "1" ]; then
  log "step A: skipped by TWIRL_SKIP_STEP_A=1"
else
  ...
  if (( attempt > MAX_RETRIES )); then
    abort "finalize stage exhausted retries"
  fi
fi
...
for orbit in "$ORBIT_1" "$ORBIT_2"; do
  for cam in 1 2 3 4; do
    ...
    $TGLC_PY "$SCRIPT_DIR/fix_tglc_quat_cadence_mismatch.py" \
...
#######################################################################
# Step D: TWIRL-FS HLSP at sector level.
#######################################################################
log "step D: TWIRL-FS HLSP"
mkdir -p "$HLSP_ROOT"
HDF5_USE_FILE_LOCKING=FALSE \
$TGLC_PY "$SCRIPT_DIR/build_twirl_hlsp.py" \
  --sector "$SECTOR" \
  --orbit-roots "$ROOT/orbit-$ORBIT_1/ffi" "$ROOT/orbit-$ORBIT_2/ffi" \
  --out-root "$HLSP_ROOT" \
  --workers 32 \
  --max-failures 250 \
  --min-success-frac 0.99 \
  >> "$SECTOR_LOG/twirl_fs_hlsp.log" 2>&1 \
  || abort "TWIRL-FS HLSP build failed"

#######################################################################
# Step F: audit + QC plots.
#######################################################################
log "step F: audit"
n_fits=$(find "$HLSP_ROOT" -name '*.fits' | wc -l)
log "  TWIRL-FS HLSP FITS count: $n_fits"
if (( n_fits == 0 )); then
  touch "$MARKERS/qc_pause.flag"
  abort "TWIRL-FS HLSP produced 0 FITS — likely lightcurve/cadence-fix upstream failure (see twirl_fs_hlsp.log). qc_pause.flag set."
fi

log "step G: QC plot sample"
mkdir -p $ROOT/qc_s${SECTOR_TAG}_gpu
$TGLC_PY "$SCRIPT_DIR/qc_plot_hlsp_sample.py" \
  --hlsp-root "$HLSP_ROOT" \
...
mkdir -p $ROOT/qc_pdf
$TGLC_PY "$SCRIPT_DIR/qc_sector_pdf.py" \
  --hlsp-root "$HLSP_ROOT" \
  --sector $SECTOR \
  --output $ROOT/qc_pdf/s${SECTOR_TAG}_twirl_fs_v2_qc.pdf \
