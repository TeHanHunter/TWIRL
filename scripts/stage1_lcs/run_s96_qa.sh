...
#   bash scripts/stage1_lcs/run_s96_qa.sh 2>&1 | tee logs/s96_qa.log
#
# After it finishes, rsync the TGLC FITS back locally:
#   rsync -av pdogpu1:/pdo/users/tehan/tglc-vanilla-s96qa/lc/ \
#       ~/PycharmProjects/TWIRL/benchmark/s96_qa/tglc_vanilla_lc/
# then locally (plotter consumes vanilla TGLC FITS by Gaia DR3 id):
#   python benchmark/plot_tglc_tica_vs_qlp_s96.py \
#       --tglc-vanilla-lc-dir benchmark/s96_qa/tglc_vanilla_lc

set -euo pipefail

TWIRL_ROOT=${TWIRL_ROOT:-/pdo/users/tehan/TWIRL}
TARGETS_JSON=${TARGETS_JSON:-$TWIRL_ROOT/benchmark/s96_qa/targets.json}
...
"$TGLC_PYTHON" "$TWIRL_ROOT/scripts/stage1_lcs/run_s96_qa_vanilla.py" \
